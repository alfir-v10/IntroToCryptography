...
#wei_r = random.weibullvariate(0.1, 0.2)
...
def run_model_weib():
    ...
    if nums == '1':
        ...
        new_posled = []
        for r in s_otr:
            new_posled.append(np.round(pow(-np.log(1-r)*pow(lam, kam), 1/kam), 3))
        int_count = 450
        exp = []
        for x in np.arange(0., 10, 0.01):
            exp.append(((kam / lam)*(pow(x/lam, kam-1)) * np.exp(-pow(x / lam, kam))))
        fig, ax = plt.subplots()
        ...
        for k in end_of_end_interval_spisok[:]:
            try:
                empty_tuple.append(
                    pow(((k[1] / int_count) - sum(integrate.quad(lambda x: ((kam / lam)*(pow(x/lam, kam-1)) * np.exp(-pow(x / lam, kam))), k[0][0], k[0][1]))),
                        2) / sum(integrate.quad(lambda x: ((kam / lam)*(pow(x/lam, kam-1)) * np.exp(-pow(x / lam, kam))), k[0][0], k[0][1])))
        ...
        n = len(new_posled)
        s_exp = s[:]
        d_exp = abs(np.mean(s_exp) - lam*gamma(1+1/kam))
        ...
        sigma = lam*lam*(gamma(1+2/kam)-gamma(1+1/kam)*gamma((1+1/kam)))
        print(sigma)
        if alpha == 0.1:
